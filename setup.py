...
pyapi_compat_if_dir = os.path.dirname(os.path.abspath(__file__))

if os.path.isdir(os.path.join(pyapi_compat_if_dir, "packages")):
    print("Packages are inside this directory")
    packages_dir = os.path.join(pyapi_compat_if_dir, "packages")
else:
    parent = os.path.dirname(pyapi_compat_if_dir)
    
    if os.path.isdir(os.path.join(parent, "pyapi-compat-if")):
        print("pyapi-compat-if is a peer")
        packages_dir = parent
    else:
...
#            )
    ...
    def copy_extensions_to_source(self):
        ...
        copy_file(
            os.path.join(cwd, "build", "src", "%spyapi-compat-if%s" % (pref, ext)),
            os.path.join(package_dir, "%spyapi-compat-if%s" % (pref, ext)))
...
#                os.path.join(pyapi_compat_if_dir, 'python', 'VscTasks.cpp'),
            ],
            language="c++",
            include_dirs=[
